...
def get_geom_type(table_name, geo_col_name):
    ...
    srs_wkt_exists = (
        connection.execute(
            sa.text("select count(name) from pragma_table_info('spatial_ref_sys') where name is 'srs_wkt'")
        ).scalar()
        == 1
    )
    # spatialite 3
    if srs_wkt_exists:
        return connection.execute(
            sa.text(f"SELECT type from geometry_columns where f_table_name='{table_name}'")
        ).fetchone()[0]
    else:
        geom_type_num = connection.execute(
            sa.text(f"SELECT geometry_type from geometry_columns where f_table_name='{table_name}'")
        ).fetchone()[0]
...
def add_geometry_column(table: str, name: str, srid: int, geometry_type: str):
    ...
    query = f"SELECT AddGeometryColumn('{table}', '{name}', {srid}, '{geometry_type}', 'XY', 1);"
...
def transform_column(table_name, srid):
    ...
    op.execute(sa.text(f"SELECT RecoverGeometryColumn('{table_name}', " f"'geom', {srid}, '{geom_type}', 'XY')"))
