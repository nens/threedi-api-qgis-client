...
DEFAULT_VALUES = {
    "lateral_1d": {"time_units": "'minutes'", "interpolate": "0", "offset": "NULL", "units": "'m3/s'"},  # false
    "lateral_2d": {"time_units": "'minutes'", "interpolate": "0", "offset": "NULL", "units": "'m3/s'"},  # false
    "boundary_condition_1d": {
        "time_units": "'minutes'",
        "interpolate": "1",  # true
    },
...
def add_geometry_column(table: str, geocol: Column):
    ...
    geotype = geocol.type
    query = f"SELECT AddGeometryColumn('{table}', '{geocol.name}', {geotype.srid}, '{geotype.geometry_type}', 'XY', {int(not geocol.nullable)});"
...
def rename_columns(table_name: str, columns: List[Tuple[str, str]]):
    ...
    if not all(e in old_columns_list for e in columns_dict):
        raise ValueError(
            f"Cannot rename columns {columns_dict.keys()} in table {table_name}; table does not contain all these columns"
        )
    ...
    op.execute(
        sa.text(
            f"INSERT INTO {temp_name} ({','.join(new_columns_list)}) SELECT {','.join(old_columns_list)} from {table_name};"
        )
    )
    ...
    if table_name is not GEOMETRY_TYPES:
        op.execute(
            sa.text(
                f"""SELECT RecoverGeometryColumn('{table_name}', 'geom', 4326, '{GEOMETRY_TYPES[table_name]}', 'XY')"""
            )
        )
...
def upgrade():
    ...
    for table, column in (("lateral_1d", "geom"), ("boundary_condition_1d", "geom")):
