...
def remove_invalid_rows_from_v2_control_measure_map():
    ...
    if len(invalid_rows) > 0:
        ...
        msg = (
            f"Could not migrate the following rows from v2_control_measure_map because "
...
def fix_geometry_columns():
    GEO_COL_INFO = [(row[0], row[1].name, row[1].type.geometry_type) for row in ADD_COLUMNS if row[1].name == "geom"]
...
def upgrade():
    ...
    move_setting("model_settings", "use_structure_control", "simulation_template_settings", "use_structure_control")
