...
from sqlalchemy import Column, Float, Integer, String, Text, func, select
...
DELETE_TABLES = ["v2_cross_section_definition", "v2_floodfill", "v2_connection_nodes"]
...
def add_geometry_column(table: str, geocol: Column):
    ...
    geotype = geocol.type
    query = f"SELECT AddGeometryColumn('{table}', '{geocol.name}', {geotype.srid}, '{geotype.geometry_type}', 'XY', 1);"
...
def fix_geometry_columns():
    ...
    for table in tables:
        ...
        op.execute(sa.text(f"SELECT RecoverGeometryColumn('{table}', " f"'geom', {4326}, '{geom_type}', 'XY')"))
...
def extend_cross_section_definition_table():
    ...
    for row in res:
        ...
        data_to_insert.append(
            {"id": id, "shape": shape, "width": width, "height": height}
        )  # insert data into the temp table
    for data in data_to_insert:
        conn.execute(
            sa.text(
                f"""INSERT INTO {Temp.__tablename__} (id, cross_section_shape, cross_section_width, cross_section_height)
            VALUES (:id, :shape, :width, :height)"""
            ),
            data,
        )  # Pass parameters as dictionary directly
...
def migrate_cross_section_definition_from_temp(target_table: str, cols: List[Tuple[str, str]], def_id_col: str):
    ...
    set_query = ",".join(
        f"{cname} = (SELECT {cname} FROM {Temp.__tablename__} WHERE id = {target_table}.{def_id_col})"
        for cname, _ in cols
    )
    op.execute(
        sa.text(
...
def migrate_cross_section_definition_to_location():
    ...
    migrate_cross_section_definition_from_temp(
        target_table="v2_cross_section_location", cols=cols, def_id_col="definition_id"
    )
...
def migrate_cross_section_definition_to_object(table_name: str):
    ...
    migrate_cross_section_definition_from_temp(
        target_table=table_name, cols=cols, def_id_col="cross_section_definition_id"
    )
...
def fix_geom_for_culvert():
    ...
    # Create new table (temp), insert data, drop original and rename temp to table_name
    new_col_str = ",".join(
        ["id INTEGER PRIMARY KEY NOT NULL"] + [f"{cname} {ctype}" for cname, ctype in zip(col_names, col_types)]
    )
    op.execute(sa.text(f"CREATE TABLE {new_table_name} ({new_col_str});"))
    ...
    op.execute(
        sa.text(
            f"INSERT INTO {new_table_name} (id, {','.join(col_names)}, the_geom) "
...
def create_connection_node():
    ...
    rename_map = {
        "surface_level": "manhole_surface_level",
    ...
    set_items = ",\n".join(
        f"""{rename_map.get(col_name, col_name)} = (
...
def fix_material_id():
    ...
    for table in material_id_tables:
        op.execute(
            sa.text(
                f"UPDATE {table} SET material_id = CASE material_id "
