...
ADD_TABLES = {
    ...
    "interception": [Column("interception", Float), Column("interception_file", String)],
    "physical_settings": [Column("use_advection_1d", Integer), Column("use_advection_2d", Integer)],
    "simulation_template_settings": [Column("name", String), Column("use_0d_inflow", Integer)],
    "time_step_settings": [
...
COPY_FROM_GLOBAL = {
    "simulation_template_settings": [("name", "name"), ("use_0d_inflow", "use_0d_inflow")],
    "time_step_settings": [
...
def set_use_from_settings_id():
    for settings_col, settings_id, settings_table in GLOBAL_SETTINGS_ID_TO_BOOL:
        ...
        op.execute(f"DELETE FROM {settings_table} WHERE id != " f"(SELECT {settings_id} FROM model_settings);")
    ...
    for table, col in raster_paths:
        ...
        if len(result) == 1:
            ...
            if isinstance(file_path, str) and len(file_path) > 0:
                ...
                op.execute(
                    sa.text(f"UPDATE {table} SET {col} = :new_value WHERE id = :row_id").bindparams(
                        new_value=file, row_id=id
                    )
                )
...
def set_flow_variable_values():
    flow_var_dict = {"wet_cross-section": "wet_cross_section", "waterlevel": "water_level"}
